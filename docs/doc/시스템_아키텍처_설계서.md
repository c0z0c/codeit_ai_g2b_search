...
graph TB
    ...
    subgraph STORAGE["데이터 저장소"]
        ...
        FAISS[("통합 Vector Store<br/>Single FAISS Index<br/>vectorstore.faiss")]
    end
...
class EmbeddingProcessor:
    ...
    def add_to_faiss_index(self, chunks: List[str], metadata: List[Dict]) -> None
```
...
flowchart TD
    ...
    E2 --> F{통합 FAISS<br/>인덱스 존재?}
    F -->|Yes| F1[기존 인덱스 로드]
    F -->|No| F2[새 인덱스 생성]
    F1 --> G[벡터 추가<br/>add_embeddings]
    F2 --> G
    G --> G1[통합 인덱스 저장<br/>vectorstore.faiss]
    G1 --> H[embeddings.db<br/>메타데이터 저장]
    H --> H1[chunk_mapping 테이블<br/>파일명, 페이지 번호 포함]
    H1 --> I[처리 완료]

    style A stroke-width:2px,stroke:#e1f5ff
    style I stroke-width:2px,stroke:#c8e6c9
    style E stroke-width:2px,stroke:#fff9e1
    style G stroke-width:2px,stroke:#ffe0b2
```

**통합 FAISS 관리 특징**:
- **단일 인덱스**: 모든 문서의 임베딩을 하나의 FAISS 인덱스에 저장
- **메타데이터 추적**: chunk_mapping 테이블에 파일명, 페이지 번호 저장
...
```mermaid
flowchart TD
    A[사용자 쿼리 입력] --> B[통합 FAISS 인덱스<br/>로드]
    B --> C[쿼리를 벡터로 임베딩<br/>OpenAI API]
    C --> D[FAISS 유사도 검색<br/>L2 distance]
    D --> D1[전체 문서 대상 검색]
    D1 --> D2[top_k개 청크 검색]
    D2 --> D3[distance 및 index 반환]
    D3 --> E[vector_index로<br/>chunk_mapping 조회]
    E --> E1[파일명 조회]
    E --> E2[페이지 번호 조회]
    E --> E3[청크 텍스트 조회]
    E1 --> F[유사도 점수 계산]
    E2 --> F
    E3 --> F
    F --> F1["similarity = 1 / (1 + distance)"]
    F1 --> F2[범위: 0~1]
    F2 --> G[검색 결과 반환]
    G --> G1["[chunk_text, file_name,<br/>page_number, similarity, ...]"]
...
```python
{
    'chunk_id': int,
    'chunk_text': str,
    'file_name': str,        # 파일명 포함
    'file_hash': str,
    'start_page': int,       # 시작 페이지 번호
    'end_page': int,         # 종료 페이지 번호
    'similarity': float,     # 0~1 유사도 점수
...
CREATE TABLE chunk_mapping (
    ...
    start_page INTEGER,                  -- 시작 페이지 번호 (검색 결과에 포함)
    end_page INTEGER,                    -- 종료 페이지 번호 (검색 결과에 포함)
...
**주요 메서드**:
- `insert_chunk_mapping()`: 청크 매핑 저장 (파일명, 페이지 번호 포함)
- `get_chunk_by_vector_index()`: 벡터 인덱스로 청크, 파일명, 페이지 번호 조회
- `get_chunks_by_file()`: 파일별 청크 조회
- `get_total_chunks()`: 전체 청크 수 조회
...
sequenceDiagram
    ...
    EP->>EP: 텍스트 청킹
    EP->>EP: 벡터 임베딩 생성
    EP->>FAISS: 통합 FAISS 인덱스 로드/생성
    EP->>FAISS: 새 벡터 추가
    EP->>EDB: chunk_mapping 저장<br/>(파일명, 페이지 번호 포함)
    deactivate EP

    Note over FAISS: 통합 vectorstore.faiss 파일 업데이트
```
...
**목적**: 모든 문서의 임베딩을 하나의 FAISS 인덱스로 통합 관리
...
if os.path.exists(vector_path):
    ...
    vectorstore.add_embeddings(new_embeddings, metadatas)
else:
    vectorstore = FAISS.from_embeddings(embeddings, metadatas)
vectorstore.save_local(vector_path)
```

**효과**:
- 전체 문서를 대상으로 통합 검색 가능
- 파일명과 페이지 번호를 통한 정확한 출처 추적
- 벡터 인덱스 관리 단순화
- 여러 문서에 걸친 관련 정보 통합 제공
...
│       ├── vectorstore.faiss       # 통합 벡터 인덱스
│       └── vectorstore.pkl         # 메타데이터
│
├── docs/                           # 문서
│   ├── doc/
...
erDiagram
    ...
    embedding_config ||--o{ chunk_mapping : "configures"
    embedding_config {
        int id PK
        int chunk_size
        int chunk_overlap
...
**주요 변경사항 (v2.0)**:
- 마크다운 변환 및 토큰 계산 기능 제거 (문서 처리 단순화)
- 통합 FAISS 인덱스 관리로 변경 (여러 파일을 하나의 인덱스로 관리)
- 검색 결과에 파일명과 페이지 번호 포함
- 데이터베이스 스키마 간소화 (embedding_meta → embedding_config)
